...
ActionSaveTo (int argc, char **argv, Coord x, Coord y)
{
  ...
  if ( ! function || strcasecmp (function, "Layout") == 0)
    {
      if (EMPTY_STRING_P (PCB->Filename))
        {
          Message (_("Filename is not known. Use SaveTo(LayoutAs)\n"));
	  return 0;
	}
      if (!hid_file_format_capable (PCB->Fileformat, HID_FFORMAT_SAVEABLE))
        {
          Message (_("Current format \"%s\" does not suport Save operation. Use SaveTo(LayoutAs)\n"), PCB->Fileformat);
	  return 0;
	}
  ...
  if (strcasecmp (function, "LayoutAs") == 0)
    {
      if (!hid_file_format_capable (format, HID_FFORMAT_SAVEABLE))
        {
          Message (_("Specified \"%s\" format does not suport Save operation.\n"), format);
	  return 0;
	}
