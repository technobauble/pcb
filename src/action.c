...
/* NotifyMode, ReleaseMode, SaveMode, RestoreMode are now in ModeManager.cpp */
...
/* click_cb timer callback removed - ArrowMode now uses distance-based drag detection.
 * ReleaseMode_Legacy has been removed - release handling now in ArrowMode.cpp
 * and ModeManager.cpp's ReleaseMode() wrapper.
 */
...
/* NotifyMode_Legacy has been removed - all modes now use C++ State Pattern.
 * See src/actions/modes/ for mode implementations.
 */


/* --------------------------------------------------------------------------- */

/* Rest of ARROW_MODE case removed - implemented in ArrowMode.cpp */
/* Rest of VIA_MODE case removed - implemented in ViaMode.cpp */
/* Rest of ARC_MODE case removed - implemented in ArcMode.cpp */
/* Rest of LOCK_MODE case removed - implemented in LockMode.cpp */
/* Rest of THERMAL_MODE case removed - implemented in ThermalMode.cpp */
/* Rest of LINE_MODE case removed - implemented in LineMode.cpp */
/* Rest of RECTANGLE_MODE case removed - implemented in RectangleMode.cpp */
/* Rest of TEXT_MODE case removed - implemented in TextMode.cpp */
/* Rest of POLYGON_MODE case removed - implemented in PolygonMode.cpp */
/* Rest of POLYGONHOLE_MODE case removed - implemented in PolygonHoleMode.cpp */
/* Rest of PASTEBUFFER_MODE case removed - implemented in PasteBufferMode.cpp */
/* Rest of REMOVE_MODE case removed - implemented in RemoveMode.cpp */
/* Rest of ROTATE_MODE case removed - implemented in RotateMode.cpp */
/* Rest of COPY_MODE case removed - implemented in CopyMode.cpp */
/* Rest of MOVE_MODE case removed - implemented in MoveMode.cpp */
/* Rest of INSERTPOINT_MODE case removed - implemented in InsertPointMode.cpp */

/* NOTE: The above comments are placeholders - the actual legacy code that was
 * here (~815 lines) has been completely removed. Each mode is now a self-contained
 * C++ class in src/actions/modes/.
 */
...
ActionMode (int argc, char **argv, Coord x, Coord y)
{
  ...
  if (function)
	...
	case F_None:
	  ...
	  break;
	case F_Cancel:
	  /* Cancel current operation but stay in same mode */
	  CancelMode ();
	  break;
	case F_Escape:
	  /* If mode is idle (no operation in progress), switch to ARROW_MODE.
	   * Otherwise, cancel current operation and stay in mode. */
	  if (Settings.Mode == ARROW_MODE)
	    {
	      /* Already in arrow mode, nothing to do */
	    }
	  else if (IsModeIdle ())
	    {
	      /* No operation in progress - switch to arrow mode */
	      SetMode (ARROW_MODE);
	    }
	  else
	    {
	      /* Operation in progress - cancel it but stay in mode */
	      CancelMode ();
	    }
	  break;
