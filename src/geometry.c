...
/* The pcb-specific type settings happen in this header at the moment, so we
 * include this here.  */
...
vec_dot (Vec va, Vec vb)
{
   ...
   * than CT.  I think this is a good thing and not something we want to
   * mask.  */
    ...
    Point           *pii )
{
  ...
  /* Check if the center of the circle is on the rectangle.  Note: this catches
  ...
  Point const *ca = rect->corner;   /* Convenience alias for Corner Array */

  /* Check if any of the corners of the rect lie inside circ.  Note: this
   * catches the case where the rectangle is entirely inside the circle. */
  for ( int ii = 0 ; ii < 4 ; ii++ ) {
    ...
    Point              intersection[2] )
{
  ...
  /* Translated end point coordinates st circ is relatively at (0, 0) */
   ...
   * likely to be parallel to the axes, we make sure to check the wider
   * span.  */
  if ( ABS (dx) >= ABS (dy) ) {
