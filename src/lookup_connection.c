...
/*****************************************************************************
 *
 * New implementation using object_list and DRCObject
 *
 *****************************************************************************/
...
{
  rtree_t * search_tree;
  int (*callback)(const * BoxType, void *data);
} SearchDefinition;

/*
 * lookup_connections_to_object
 *
 * Find other objects that make electrical connections to passed object.
 *
 * The depth parameter indicates how far along the trace should go. 
 * If depth = 1, only objects immediately touching the passed object will be 
 * added to the list. If depth = 2, objects touching objects that
 * immediately touch the original object will be added, etc. If depth <= 0,
 * the trace will go until no new objects were added.
 *
 * Returns a pointer to an object_list of DRCObjects. The calling function
 * takes ownership of this list.
 * */
object_list * 
lookup_connections_to_object (DRCObject * seed, int depth)
{
  /* This is the list we'll return containing the objects we find. */
  ...
  for (d=1; d <= depth; d++)
  {
         ...
         i++)
    {
      ...
      search_box = ((AnyObjectType*)obj)->BoundingBox;
