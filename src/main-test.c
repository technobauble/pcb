...
#include "core_lists.h"
  ...
  // FIXME: it's a bit goofy to use printer hid here, we want a generic
  // batch mod hid or maybe the nogui hid mentioned in hid_init or something.
  ...
  // FIXME: commented out for now because we don't have lib files in place
  // when testing from the build dir so it spews warning.  But something
  // like it might be useful to make the lib available for tests.
