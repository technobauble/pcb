...
#define HID_FFORMAT_SAVEABLE	1
  ...
  typedef struct
  {
    ...
    bool (*check_version) (unsigned long, unsigned long);
    /* Functions to call to process file. PCBType definition is not available in HID, hence void * is used  */
    int (*check_function) (char *);
    int (*load_function) (void *, char *);
    int (*save_function) (void *, char *);
    /*
    Not implemented yet
    int (*load_element_function) (void *, char *);
...
{ hid_register_formats(a, sizeof(a)/sizeof(a[0])); }
  ...
  extern bool hid_file_format_capable(char *id, unsigned int capabilities);
  extern bool hid_file_format_capable_by_idx (int idx, unsigned int capabilities);
